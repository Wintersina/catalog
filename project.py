...
# logs user in, with google authentication
...
def showAllCategories():
    recently = db_session.query(Recent).order_by(
        desc(Recent.created_date)).all()
...
def catalogItems(catalog_name):
    ...
    get_all_items_for_category = db_session.query(Items).filter_by(
        category_id=get_category_id.id)
    return render_template(
        'listitems.html',
        a_c=all_catagories,
        a_i=get_all_items_for_category,
        c_n=get_category_id.name
    )
...
def aboutItem(catalog_name, item_name):
    ...
    item = db_session.query(Items).filter_by(
        title=item_name, category_id=get_category_id.id).first()
    return render_template('aboutitem.html',
                           item=item,
...
def editItem(catalog_name, item_name):
    ...
    get_category_id = db_session.query(Categories).filter_by(
        name=catalog_name).one()
    editItem = db_session.query(Items).filter_by(
        title=item_name,
        category_id=get_category_id.id).first()
...
def deleteItem(catalog_name, item_name):
    ...
    get_category_id = db_session.query(Categories).filter_by(
        name=catalog_name).one()
    deleteitem = db_session.query(Items).filter_by(
        title=item_name,
        category_id=get_category_id.id).first()
    deleteRecent = db_session.query(Recent).filter_by(
        item_id=deleteitem.id).first()
    if 'username' not in login_session:
        print(login_session)
        return "Please log in :)"
        ...
        if request.method == 'POST':
            ...
            output = render_template('itemdelete.html',
                                     category=get_category_id,
                                     item=deleteitem,
                                     r=results)
            return output
...
def jsonItemListForCatelog(catalog_name):
    ...
    items = db_session.query(Items).filter_by(
        category_id=get_category_id.id
    ).all()
...
def jsonAll():
    ...
    return jsonify(
        Categories=[i.serialize for i in get_category],
        Items=[i.serialize for i in get_all_items]
    )
